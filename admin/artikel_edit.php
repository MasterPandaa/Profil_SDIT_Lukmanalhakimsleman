...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($judul != $artikel['judul']) {
        ...
        $query = "SELECT COUNT(*) as count FROM artikel WHERE slug = ? AND id != ?";
        $stmt = mysqli_prepare($conn, $query);
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === 0) {
        ...
        if (in_array(strtolower($ext), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target)) {
                ...
                if (!empty($artikel['foto']) && file_exists('../assets/images/blog/' . $artikel['foto'])) {
                    unlink('../assets/images/blog/' . $artikel['foto']);
                }
                $foto = $newname;
            } else {
    ...
    if (empty($message)) {
        $query = "UPDATE artikel SET judul = ?, slug = ?, konten = ?, foto = ?, kategori = ?, status = ?, tanggal = ?, penulis = ?, updated_at = NOW() WHERE id = ?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "ssssssssi", $judul, $slug, $konten, $foto, $kategori, $status, $tanggal, $penulis, $id);
                            ...
                            <div class="col-md-4">
                                ...
                                <?php if (!empty($artikel['foto'])): ?>
                                    <div class="mb-3">
                                        ...
                                        <div>
                                            <img src="../assets/images/blog/<?php echo htmlspecialchars($artikel['foto']); ?>" alt="Gambar Artikel" class="img-thumbnail img-preview" style="max-width: 200px;">
                                        </div>
                                    </div>
                                <?php endif; ?>
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="kategori" name="kategori" required>
                                        ...
                                        <option value="Kegiatan" <?php echo ($artikel['kategori'] == 'Kegiatan') ? 'selected' : ''; ?>>Kegiatan</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="status" name="status" required>
                                        ...
                                        <option value="published" <?php echo ($artikel['status'] == 'published') ? 'selected' : ''; ?>>Publikasikan</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label for="tanggal" class="form-label">Tanggal</label>
                                    <input type="date" class="form-control" id="tanggal" name="tanggal" value="<?php echo htmlspecialchars($artikel['tanggal']); ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label for="penulis" class="form-label">Penulis</label>
                                    <input type="text" class="form-control" id="penulis" name="penulis" value="<?php echo htmlspecialchars($artikel['penulis']); ?>" required>
                                </div>
                                <div class="mb-3">
                                    <small class="text-muted">Terakhir diperbarui: <?php echo date('d F Y H:i', strtotime($artikel['updated_at'])); ?></small>
                                </div>
                            </div>
                        </div>
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="artikel.php" class="btn btn-outline-secondary">Batal</a>
